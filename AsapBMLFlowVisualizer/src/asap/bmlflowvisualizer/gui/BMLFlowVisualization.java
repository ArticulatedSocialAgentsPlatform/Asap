...
public class BMLFlowVisualization extends JPanel
{
    ...
    private Map<String, BMLBlock> bmlBlocks = Collections.synchronizedMap(new HashMap<String, BMLBlock>());
    ...
    public BMLFlowVisualization(BMLFlowVisualizerPort port, Map<String, BMLBlock> bmlBlocks)
    {
        this.port = port;
        ...
        JLayer<ScrollPanel> jlayer = new JLayer<ScrollPanel>(histPanel, histPanelUI);
        ...
        scrollPane.getHorizontalScrollBar().addAdjustmentListener(new AdjustmentListener()
        {
            ...
            public void adjustmentValueChanged(AdjustmentEvent e)
            {
                ...
                {
                    if (lockScrolling) brm.setValue(brm.getMaximum());
                }
                else
                {
                    lockScrolling = ((brm.getValue() + brm.getExtent()) == brm.getMaximum());
                }
            }
        });
        ...
        Timer timer = new Timer(PLAY_INTERVAL, createPlayer());
        timer.start();
    ...
    private ScrollPanel buildHistoryPanel()
    {
        ...
        histScheduledPanel.addMouseListener(new ClickListener(DBCLICK_INTERVAL)
        {
            @Override
            public void singleClick(MouseEvent e)
            {
    ...
    private JPanel buildDetailPanel()
    {
        ...
        detailPlannedPanel.addMouseListener(new ClickListener(DBCLICK_INTERVAL)
        {
        ...
        detailScheduledPanel.addMouseListener(new ClickListener(DBCLICK_INTERVAL)
        {
    ...
    /**
     * Creates the ActionListener that handles to periodic update of the visualization
     * 
        ...
        return new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                if (running && !feedbackOnly.isSelected())
                {
                    ...
                    long time = System.currentTimeMillis() - port.getFirstTimestamp();
    ...
    private void updateDetailVisualisation(long timestamp)
    {
        ...
        for (BMLBlock b : bmlBlocks.values())
        {
            if ((b.getPlanStart() != -1 && b.getPlanStart() <= timestamp) && (b.getPlanEnd() == -1 || b.getPlanEnd() >= timestamp))
            {
                plannedBlocks.add(b);
            }
            else
            {
                b.setPosition(new Point(0, 0), 0, VisualisationField.DetailPlanned);
            }
            if ((b.getSchedStart() != -1 && b.getSchedStart() <= timestamp) && (b.getSchedEnd() == -1 || b.getSchedEnd() >= timestamp))
            {
                scheduledBlocks.add(b);
            }
            else
            {
                b.setPosition(new Point(0, 0), 0, VisualisationField.DetailScheduled);
            }
            if ((b.getPlayStart() != -1 && b.getPlayStart() <= timestamp) && (b.getPlayEnd() == -1 || b.getPlayEnd() > timestamp))
            {
                scheduledBlocks.add(b);
                playingBlocks.add(b);
            }
    ...
    {
        BufferedImage img = new BufferedImage(detailPlannedPanel.getWidth(), detailPlannedPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
        ...
        for (BMLBlock b : blocks)
        {
            ...
            g.drawString(trunkStringToFit(b.getId(), BLOCK_WIDTH, metric), x + 5, y + BLOCK_HEIGHT / 2 + 2);
            b.setPosition(new Point(x, y), BLOCK_WIDTH, VisualisationField.DetailPlanned);
     ...
     */
    private void drawOnDetailScheduled(ArrayList<BMLBlock> playingBlocks, ArrayList<BMLBlock> scheduledBlocks, long timestamp)
    {
        ...
        for (BMLBlock b : scheduledBlocks)
        {
            ...
            BMLABMLBehaviorAttributes bmlaAttr = b.getBb().getBMLBehaviorAttributeExtension(BMLABMLBehaviorAttributes.class);
            if (bmlaAttr != null)
            {
        ...
        BufferedImage img = new BufferedImage(newWidth, detailScheduledPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
        Graphics2D g = img.createGraphics();
        ...
        for (Entry<String, Point> entry : layout.entrySet())
        {
            ...
            g.setColor(Color.BLACK);
            FontMetrics metric = g.getFontMetrics();
            g.drawString(trunkStringToFit(entry.getKey(), w, metric), x + 5, y + h / 2 + 2);
            bmlBlocks.get(entry.getKey()).setPosition(new Point(x, y), w, VisualisationField.DetailScheduled);
        }
        ...
        for (Edge<String> e : edges)
        {
            ...
            g.drawLine(b1.getPosition(4).x, b1.getPosition(4).y + BLOCK_HEIGHT / 2, b2.getPosition(4).x + BLOCK_WIDTH, b2.getPosition(4).y
                    + BLOCK_HEIGHT / 2);
        }
        g.dispose();
    ...
    private void drawHist(BufferedImage img, VisualisationField field)
    {
        ...
        for (BMLBlock b : blocks)
        {
            ...
            if (start > 0)
            {
                ...
                g.drawString(trunkStringToFit(b.getId(), xEnd - xStart, metric), xStart + 2, y + BLOCK_HEIGHT / 2 + 2);
    ...
    {
        BufferedImage contImg = new BufferedImage(curHistPanelWidth, histPlayedPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    {
        BufferedImage contImg = new BufferedImage(curHistPanelWidth, histScheduledPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    {
        BufferedImage contImg = new BufferedImage(curHistPanelWidth, histPlannedPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    public void updateHistory(long time)
    {
        ...
        histPanel.setPreferredSize(new Dimension(curHistPanelWidth, histPanel.getHeight()));
        int posX = (int) (curTimestamp * 0.001 * pixPerSec) + 1;
        histPanelUI.setTimeLinePos(posX);
        timeField.setText(String.valueOf(curTimestamp * 0.001));
    }
