...
public class PictureEngineLoader implements EngineLoader
{
    ...
    private void constructEngine(XMLTokenizer tokenizer)
    {
        ...
        PlanPlayer planPlayer = new SingleThreadedPlanPlayer<TimedPictureUnit>(theVirtualHuman.getElckerlycRealizer().getFeedbackManager(),
                planManager);
        player = new DefaultPlayer(planPlayer);
        PicturePlanner planner = new PicturePlanner(theVirtualHuman.getElckerlycRealizer().getFeedbackManager(), pictureBinding,
                planManager);
