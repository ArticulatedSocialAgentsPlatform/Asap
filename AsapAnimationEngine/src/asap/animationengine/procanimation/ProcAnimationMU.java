...
public class ProcAnimationMU extends XMLStructureAdapter implements AnimationUnit {
	...
	public ProcAnimationMU deepCopy() {
		...
		for (KeyframeMU kmu : keyFrameMUs) {
			copy.addSkeletonInterpolator(new SkeletonInterpolator(kmu
					.getSkeletonInterpolator(), null));
		}
	 ...
	 *            z formula
	 */
	public void setRotation(String name, String xForm, String yForm,
			String zForm) throws ParseException {
	 ...
	 *            z formula
	 */
	public void setEndEffector(String name, String xForm, String yForm,
			String zForm, String sForm, boolean isLocal) {
	 ...
	 *            lefthand, right hand
	 */
	public void getPositions(float lf[], float rf[], float lh[], float rh[],
			float root[], double swivels[]) {
		if (rootEff != null) {
	...
	private void setupEndEffectors() {
		...
		rightHandEff = null;
		for (EndEffector eff : endeffector) {
			if (eff.getTarget().equals("root")
					|| eff.getTarget().equals(Hanim.HumanoidRoot)) {
				rootEff = eff;
	...
	private void filterBodyParts() {
		...
		for (VJoint v : bodyParts) {
			...
			if (keyframes.get(v.getSid()) != null)
				continue;
			if ((v.getSid() == Hanim.r_shoulder || v.getSid() == Hanim.r_elbow)
					&& rightHandEff != null)
				continue;
			if ((v.getSid() == Hanim.l_shoulder || v.getSid() == Hanim.l_elbow)
					&& leftHandEff != null)
				continue;
			if ((v.getSid() == Hanim.r_hip || v.getSid() == Hanim.r_knee || v
					.getSid() == Hanim.r_ankle) && rightFootEff != null)
				continue;
			if ((v.getSid() == Hanim.l_hip || v.getSid() == Hanim.l_knee || v
					.getSid() == Hanim.l_ankle) && leftFootEff != null)
				continue;
			deleteList.add(v);
		}
	...
	private void findMissingParameters() {
		...
		// formula's
		{
			...
			if (containsOnlyLetters(par) && !parameterExists(par)
					&& !par.equals("t") && !isConstantParameter(par)) // If the
			// parameter
			// is not
			// defined
			{
	...
	private void play(double t, VJoint v) {
		...
		} else if ((v.getSid() == Hanim.l_hip || v.getSid() == Hanim.l_knee || v
				.getSid() == Hanim.l_ankle) && leftFootEff != null) {
		...
		} else if ((v.getSid() == Hanim.r_shoulder || v.getSid() == Hanim.r_elbow)
				&& rightHandEff != null) {
		...
		} else if ((v.getSid() == Hanim.l_shoulder || v.getSid() == Hanim.l_elbow)
				&& leftHandEff != null) {
	...
	public void decodeContent(XMLTokenizer tokenizer)
		...
		while (!tokenizer.atETag()) {
			if (tokenizer.atSTag("EndEffector")) {
				...
				if (!(rot.getRotationFormula(0) == null
						&& rot.getRotationFormula(1) == null && rot
						.getRotationFormula(2) == null)) {
					if (!(rot.getRotationFormula(0).equals("")
							&& rot.getRotationFormula(1).equals("") && rot
							.getRotationFormula(2).equals(""))) {
				...
				throw new RuntimeException(
						"CutPositions have been deprecated, please remove from XML file");
			}
		}
	}
	...
	@Override
	public TimeAnimationUnit createTMU(FeedbackManager bfm, BMLBlockPeg bbPeg,
