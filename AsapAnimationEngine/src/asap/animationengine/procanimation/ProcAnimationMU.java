...
{
    // private static Logger logger = LoggerFactory.getLogger(ProcAnimationMU.class.getName());

    public Id id = null;

    // private ArrayList<KeyPosition> keypositions = new ArrayList<KeyPosition>();
    ...
    public void set(ProcAnimationMU ani)
    {
        ...
        for (KeyframeMU kmu : ani.keyFrameMUs)
        {
            ...
            // skelInterpolators.add(new SkeletonInterpolator(ip, ip.getTargetParts()));
        }
     ...
     * @param xForm x formula
     * @param yForm y formula
     * @param zForm z formula
     */
     ...
     * 
     * @param ik the IKAnimation to copy from
     */
     ...
     * 
     * @param the new list of EndEffectors
     */
     ...
     * 
     * @param target the sid of the endeffector to obtain
     ...
     * 
     * @param lf output: left foot vector
     * @param rf output: right foot position
     * @param lh output: left hand position
     * @param rh output: right hand position
     * @param root output: root position
     * @param swivels output: swivel rotations of respectively left foot, rightfoot, lefthand, right hand
     */
    ...
    /**
     * Links the ProcAnimation to a body and a set of parameters Filters out the joints to animate based on the set Rotations, end effectors,
     * keyframes and skeletoninterpolators
     */
    ...
    private void filterBodyParts()
    {
        ...
        if (bodyPartFilter.size() > 0)
        {
            for (VJoint v : bodyParts)
            {
                ...
                // TODO: remove endeffectors completely if not all their joints are in the filter
    ...
    /**
     * Play the IKAnimation at time 0 &lt t &lt 1 First plays all skeletoninterpolators, then sets the joints starting at the root as specified by IK
     * and keyframes
     * 
     * @param t 0 &lt;= t &lt;= 1 the time position
     */
    @Override
    ...
    /**
     * @param maxDuration The new maxDuration in seconds
     */
    ...
    /**
     * @param minDuration The minDuration to set, in seconds
     */
    ...
    /**
     * @param prefDuration the prefered duration (in seconds) of this ProcAnimation
     */
    ...
    /*
     * private void lowPassFilter(SkeletonInterpolator ski) { int rots = ski.getConfigSize()-3; rots/=4;
     * 
     * //Filter float Fin[]=new float[ski.getConfigList().size()*ski.getConfigSize()]; float Fout[]=new
     * float[ski.getConfigList().size()*ski.getConfigSize()]; for(int i=0;i<ski.getConfigList().size();i++) { float config[] =
     * ski.getConfigList().getConfig(i); System.arraycopy(config, 0, Fin, i*ski.getConfigSize(), ski.getConfigSize()); }
     * //System.out.println("config size: "+ski.getConfigSize());
     * 
     * //flipping quats, should not really be needed since this is actually taken care of in the .XML for(int i=1;i<ski.getConfigList().size()-1;i++)
     * { for(int j=0;j<rots;j++) { int indexPrev = ski.getConfigSize()*(i-1)+j*4; int indexCur = ski.getConfigSize()*i+j*4; int indexNext =
     * ski.getConfigSize()*(i+1)+j*4; if(Vec4f.dot(Fin, indexPrev, Fin, indexCur)<0) { Vec4f.scale(-1, Fin,indexCur);
     * logger.debug("Flipping cur quat"); } if(Vec4f.dot(Fin, indexCur, Fin, indexNext)<0) { Vec4f.scale(-1, Fin,indexNext);
     * logger.debug("Flipping next quat"); } } } double frameTime = ski.getConfigList().getTime(1)-ski.getConfigList().getTime(0); if(
     * (1d/frameTime)<15)return; //Winters.winters(Fin, 15, 120, ski.getConfigSize(), Fout); Winters.winters(Fin, 12, (float)(1d/frameTime),
     * ski.getConfigSize(), Fout);
     * 
     * logger.info("Filtering fs: {}",(float)(1d/frameTime)); //Winters.winters(Fin, 15, 30, ski.getConfigSize(), Fout);
     * 
     * for(int i=0;i<ski.getConfigList().size();i++) { for(int j=0;j<rots;j++) { int index = i*ski.getConfigSize()+3+j*4; float length =
     * Vec4f.length(Fout,index); if(Math.abs(length-1)>0.01) { logger.debug("Length "+length+" at frame "+i+" joint "+j+" index "+index); }
     * Vec4f.normalize(Fout,i*ski.getConfigSize()+3+j*4); } }
     * 
     * 
     * //copy back to interpolator for(int i=0;i<ski.getConfigList().size();i++) { System.arraycopy(Fout, i*ski.getConfigSize(),
     * ski.getConfigList().getConfig(i), 0, ski.getConfigSize()); } }
     */
